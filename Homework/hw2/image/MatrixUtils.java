...
public class MatrixUtils {
        ...
        for (int i =1;i<m.length;i++){
            for (int j = 0;j<m[0].length;j++) {
                if (j!=0 &&j <m[0].length-1) {
                    output[i][j] = Math.min(output[i - 1][j - 1], Math.min(output[i - 1][j], output[i - 1][j +1]));
                }else if (j == 0) {
                    output[i][j] = Math.min(output[i-1][j],output[i-1][j+1]);;
                } else{
                    output[i][j] = Math.min(output[i-1][j-1],output[i-1][j]);
                }
                output[i][j] += m[i][j];
                }
            }
    ...
    public static double[][] accumulate(double[][] m, Orientation orientation) {
        if (orientation != Orientation.VERTICAL) {
            return get(accumulateVertical(get(m)));
        } else {
            return accumulateVertical(m);
        }
    }
    public static double[][] get(double[][] m){
        double[][] output = new double[m[0].length][];
