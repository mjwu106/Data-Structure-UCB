...
class Model {
    ...
    void forNeighbors(Place place, Consumer<Place> action) {
        if (place.row > 0) {
            Place top = pl(place.row - 1,  place.col);
            action.accept(top);
        }
        if (place.col > 0) {
            Place left = pl(place.row,  place.col - 1);
            action.accept(left);
        }
        if (place.row < _height - 1) {
            Place bottom = pl(place.row + 1,  place.col);
            action.accept(bottom);
        }
        if (place.col < _width - 1) {
            Place right = pl(place.row,  place.col + 1);
            action.accept(right);
        }
    }
    ...
    final boolean solved() {
        return (_active.size() == _height * _width);
    }
    ...
    /**forNeighbors(start, (pl)-> {
    if (_cells[pl.row][pl.col] == color & result.contains(pl) == false) {
    result.add(pl);
    findRegion(pl,color, result);
    }});
    return result;*/
    ...
    void setActiveRegionColor(int newColor) {
        ...
        _active.clear();
        findRegion(a, newColor, _active);
        if (_lastHistory >= _current) {
            _current++;
