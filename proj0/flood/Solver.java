...
class Solver {
    ...
    static int[] chooser(Model model, int depth) {
        ...
        bestMove = -1;
        maxSize = -1;
        Model work1 = new Model(model);
        int thisMove = work1.activeRegionSize();
        for (int i = 0; i < work1.ncolors(); i++) {
            work1.setActiveRegionColor(i);
            if (thisMove < work1.activeRegionSize()) {
                bestMove = i;
                maxSize = work1.activeRegionSize();
                return new int[]{bestMove, maxSize};
            } else if (work1.activeRegionSize() == winningScore) {
                bestMove = i;
                maxSize = work1.activeRegionSize();
                work1.roundOver();
            } else {
